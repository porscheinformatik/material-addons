...
import { AfterViewInit, ChangeDetectorRef, Component, ContentChild, ElementRef, EventEmitter, Input, OnInit, Output, QueryList, TemplateRef, ViewChildren } from '@angular/core';
...
export class QuickListComponent<T extends QuickListItem> implements OnInit, AfterViewInit {
    ...
    @Input() minItems: number = undefined;
    ...
    setFocusOnAdd(): void {
        ...
        this.itemRows.changes.subscribe((els: QueryList<ElementRef>) => {
            if (els.length > this.rowCountFocus && !!els.last) {
                const firstFocusable = els.last.nativeElement
                .querySelector("button, a, input, select, textarea, [tabindex]:not([tabindex='-1'])");
                if (firstFocusable) {
    ...
    isAddAllowed(): boolean {
        return this.maxItems === undefined || (this.allItems.length < this.maxItems);
    }

    isDeleteAllowed(): boolean {
        return this.minItems === undefined || (this.allItems.length > this.minItems);
    }
}
